...
::
    ...
    def on_position_sync_reset(context, old_book, new_book):
        context.log.warning("on_book_sync_reset ---- {},{}".format(old_book, new_book))
...
.. list-table::
   ...
   * - 下单函数
     ...
     - insert_order(instrument_id, Exchange.SSE, source,account, price, volume, PriceType.Limit, Side.Buy, Offset.Open,HedgeFlag.Speculation,is_swap = false,block_id=0,parent_id=0)
   * - 行情接收函数
     - on_quote(context, quote, location)
     - on_quote(context, quote, location, dest)
   * - 逐笔成交行情
     - on_transaction(context, transaction, location)
     - on_transaction(context, transaction, location, dest)
   * - 逐笔委托行情
     - on_entrust(context, entrust, location)
     - on_entrust(context, entrust, location, dest)
   * - 订单更新
     - on_order(context, order, location)
     - on_order(context, order, location, dest)
   * - 订单成交
     - on_trade(context, trade, location)
     - on_trade(context, trade, location, dest)
   * - 历史委托
     - on_history_order(context, history_order, location)
     - on_history_order(context, history_order, location, dest)
   * - 历史成交
     - on_history_trade(context, history_trade, location)
     - on_history_trade(context, history_trade, location, dest)
   * - 历史订单查询报错回调
     - on_req_history_order_error(context, error, location)
     - on_req_history_order_error(context, error, location, dest)
   * - 历史成交查询报错回调
     - on_req_history_trade_error(context, error, location)
     - on_req_history_trade_error(context, error, location, dest)
   * - 撤单报错
     - on_order_action_error(context, error, location)
     - on_order_action_error(context, error, location, dest)
...
.. list-table::
   ...
   * - 订阅的算子器发布的数据返回
     -
     - on_synthetic_data(context, synthetic_data, location, dest)
   * - 订阅的算子器状态变化回调
     -
     - on_operator_state_change(context, operator_state_update, location)
   * - 获取账户保证金信息
     -
     - context.get_account_book(SOURCE, ACCOUNT).instrument_factors
   * - 获取订单委托信息
     -
     - context.get_account_book(SOURCE, ACCOUNT).orders
   * - 获取订单成交信息
     -
     - context.get_account_book(SOURCE, ACCOUNT).trades
   * - 获取订单输出信息
     -
     - context.get_account_book(SOURCE, ACCOUNT).order_inputs
   * - 关闭策略进程
     -
...
.. list-table::
   ...
   * - 功能描述
     ...
     - 2.5
   * - InstrumentType 代码类型
     - Warrant, Iopt    (2.5版本去除)
     - CryptoFuture, CryptoUFuture   (2.5版本增加)
   * - PriceType 报单类型
     -
     - EnhancedLimit, AtAuctionLimit, AtAuction   (2.5版本增加)
   * - OrderStatus 委托状态
     -
     - Cancelling   (2.5版本增加)
   * - TimeCondition 标识
     -
     - GFS, GTD, GFA, Unknown  (2.5版本增加)
   * - OrderStatus 委托状态
     -
     - Cancelling  (2.5版本增加)
   * - OperatorState 连接状态
...
.. list-table::
   ...
   * - Instrument  当日可交易标的信息
     - is_trading,force_update_ratio,long_margin_ratio,short_margin_ratio,conversion_rate,exchange_rate (2.5版本中去除)
     -
   * - Position 持仓信息
     -
     - static_yesterday,open_volume  (2.5版本新加)
...
**策略退出前方法** (当关闭策略的时候,策略退出之前调用这个方法，可以搭配保存数据等功能实现代码使用)
...
**进程退出前方法**  (当关闭策略的时候,策略进程退出之前调用这个方法，可以搭配保存数据等功能实现代码使用)
...
历史订单查询报错回调 on_req_history_order_error
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
历史成交查询报错回调 on_req_history_trade_error
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
同步本地与柜台持仓on_position_sync_reset
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
同步本地与柜台两融账户资金on_asset_margin_sync_reset
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

**本地维护的两融资金与柜台不一致时被调用 (60s同步一次)**

注 : 系统会每60s从柜台同步一次两融账户资金信息,并覆盖本地维护的两融账户资金信息.当本地维护的两融账户资金信息与柜台同步的不一致时此函数被调用
...
.. _on_order_action_error:

撤单报错 on_order_action_error
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
客户端状态变化回调on_broker_state_change
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
订阅算子器状态变化回调 on_operator_state_change
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
订阅算子 context.subscribe_operator
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
查询当天历史委托数据 context.req_history_order
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
查询当天历史成交数据context.req_history_trade
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
保持订阅策略历史成交标的 context.hold_book()
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

**保持订阅策略运行历史上的交易过的标的。没有调用此方法时，系统只会维护当前策略代码中通过 subscribe 方法订阅过的标的；当调用此方法后，系统会在策略启动后，根据该同名策略在历史上的交易情况，构造一份包含所有该同名策略所交易过标的，及当前策略代码中通过 subscribe 订阅的标的的账目。注意此方法仅影响标的列表，对于每个标的的具体持仓数值，是由 hold_positions() 方法来决定。**
...
范例::
    ...
    # 当前代码中重置了 subscribe，且没有调用 context.hold_book()，则该策略只会收到新订阅的标的行情，且策略收益及持仓中只包含新订阅的标的
...
保持本地所维护的策略标的持仓量清单，策略计算浮盈、市值时不同步柜台持仓量context.hold_positions()
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

**保持账目中每一标的的历史持仓。没有调用此方法时，系统会通过同步柜台查询到的持仓数据来构建策略账目，每次策略启动后，账目中所有标的的持仓都会同步为最新的柜台账户对应的持仓；当调用此方法后，系统会使用功夫内部记录的历史数据来恢复策略的账目持仓。缺省设置保证了策略账目中的持仓数据是绝对准确的，但无法反映功夫运行期间内的策略历史交易情况；如果需要获取之前运行策略时产生的历史持仓记录，则需要通过调用该方法来使系统使用本地存储的历史记录，在这种情况下，部分因素（例如在功夫系统外使用别的软件对同一账户手动交易）导致功夫内部维护的持仓记录产生偏差（如同一账户下对应的不同策略持仓汇总之和不等于账户总持仓），当发生此类偏差时，建议使用默认模式从账户持仓恢复策略持仓。**


范例::

    # 策略账目所包含的持仓标的列表取决于是否调用context.hold_book()，但每个标的的具体持仓数值则由context.hold_positions()来决定，
    # 没有调用该方法时，策略启动后的账目中的标的持仓等于所对应账户下的标的持仓：
    context.subscribe(source, ['600000', '600001'], EXCHANGE)

    # 当调用 hold_positions() 方法后，策略启动后的账目中标的持仓等于上次运行策略结束时所对应的标的持仓：
    context.hold_positions()

策略投资组合信息context.book
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
账户投资组合信息 context.get_account_book(SOURCE, ACCOUNT)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
打印WARNING级别日志 context.log.warning
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
转换时间格式(纳秒级->文本时间) context.strftime()
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
转换时间格式(文本时间->纳秒级) context.strptime()
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
时间间隔回调 context.add_time_interval
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Utils范例::
    ...
    def post_start(context):
        ...
        context.log.warning(
            " 600000 标的总仓{} , 昨仓{}".format(positions[key].volume, positions[key].yesterday_volume))
